...
In this file, we define our formalization of predicative System F
by providing:

  * Coq datatypes for kinds, terms, types and environments ;
  * a decidable structural equality predicate over types ;
  * helper functions for shifting de Bruijn indices of type and
    term variables in terms and types ;
  * helper functions for substitutions of type and term variables
    in terms and types ;
  * helper functions for accessing kinds and types of type and
    term variables in an environment ;
  * logical and decidable predicates for the wellformedness of
    types and environments ;
  * a kinding logical predicate altogether with a kinding
    algorithm ;
  * a typing logical predicate altogether with a typing
    algorithm.

The correctness properties that we expect from the decidable
predicates and the kinding and typing algorithms are stated and
proved in Correctness.v. *)
...
Section Equality.
  ...
  (** Structural equality over types. *)
...
Section EnvironmentAccess.
  ...
  Fixpoint get_typ (e : env) (i : nat) : option typ :=
    match e with
      | empty => None
      | etvar _ e' => option_map (tshift 0) (get_typ e' i)
      | evar T e' =>
        match i with
          | 0    => Some T
          | S i' => get_typ e' i'
...
Section Wellformedness.
  ...
  (** Wellformedness of types expressed as a logical property. *)
  ...
  (** Wellformedness of environments expressed as a logical property. *)
  ...
  (** Wellformedness of types expressed as a decidable predicate. *)
  ...
  (** Wellformedness of environments expressed as a decidable predicate. *)
...
Section Kinding.
  ...
  (** Kinding expressed as a logical property. *)
    ...
    match T with
      ...
      | tarr T1 T2 =>
        match kind_of e T1 with
          | None    => None
          | Some K1 =>
            option_map (fun K2 => max K1 K2)
                       (kind_of e T2)
        end
      | tall K T0 =>
        option_map (fun K' => S (max K' K))
                   (kind_of (etvar K e) T0)
    end.
...
Section Typing.
  ...
  (** Typing expressed as a logical property. *)
